...
(function(globals, doc) { //20180306 TODO
	'use strict';
	...
	exp.require = function (id) {
		...
		c = M.mod.exports = {};
		cache[id] = M.fn.call(c, exp.require, c, M.mod) || M.mod.exports;
		delete mapping[id];
		return cache[id];
	};
	...
	function parseRely(urls, fn, id){
		...
		.replace(/require\((.+?)\)/g, function(a, w){urls.push(w)});
		preload(urls);
		return urls;
	}
	//配置
	...
	function execute() {
		...
		while(initFn.length > 0){
			...
			else entry = M.fn.call(c, exp.require, c, M.mod) || M.mod.exports;
...
(function(){
	var lt = require('main/location'), r;
	lt.projectName('toolkit'); //必须,H5根目录
	r = lt.projectName()=='/' ? lt.origin() : lt.root();
	require('loader')(r + 'globalConfig.js', '', function(){initModule.config({});});
	lt = require('main/kit').screenFix();
	lt.offFix(lt.os != 'pc' ? false : 50);
